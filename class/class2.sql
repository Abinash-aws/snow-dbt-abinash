...
create schema HR.TCS ;

create schema HR.VITECH;

SHOW TABLES ;


CREATE TABLE  HR.VITECH.employees (
...
DESCRIBE  TABLE HR.VITECH.EMPLOYEES ;
...
-- TOP 5 HIGHT SALARY RECORDS / LOWEST
...
-- TO SHOW THE UNIQUE NAMES 

 SELECT DISTINCT FIRST_NAME FROM HR.TCS.EMPLOYEES  
 ORDER BY 1 ;
...
SELECT * FROM HR.TCS.EMPLOYEES
      WHERE EMPLOYEE_ID IN (200,205,100,500) ;


SELECT * FROM HR.TCS.EMPLOYEES 
       where department_id in (9,6) 
       and salary <= 10000
       and manager_id = 102 ;
...
SELECT COUNT(*) AS COUNT  FROM HR.TCS.REGIONS ;
...
SELECT TOP 5 * FROM SNOWFLAKE_SAMPLE_DATA.TPCDS_SF10TCL.CUSTOMER ;

SELECT * FROM HR.TCS.REGIONS ;
